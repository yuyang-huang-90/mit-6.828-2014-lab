	...
	if (filebno < NDIRECT){
		*ppdiskbno = &f->f_direct[filebno];
		...
		if (!f->f_indirect) {
			...
			uint32_t *new_block_addr = diskaddr(new_blockno);
			memset((void *) new_block_addr, 0, BLKSIZE);
			*ppdiskbno = &new_block_addr[filebno - NDIRECT];
		} else {
			uint32_t *block_addr = diskaddr(f->f_indirect);
